...
function Read-VerifiedCandidate([string]$RepoRoot, [string]$ManifestPath, [switch]$AllowUnaccepted) {
    ...
    if ((-not $AllowUnaccepted -and $candidate.acceptancePassed -ne $true -and $candidate.offlineRepairInstallable -ne $true) -or -not $candidate.version -or @($candidate.files).Count -lt 4) {
    ...
    if (-not (Test-Path -LiteralPath $runtimePython)) { throw 'The matching workspace Python runtime is missing.' }
